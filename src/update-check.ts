...
export function renderUpdateCheckScript(options: UpdateCheckOptions): string {
  ...
  const nudgeArgs =
    format === "claude" ? '"$latest" "$INSTALLED"' : '"$INSTALLED" "$latest"';
  return `#!/bin/sh
...
export function applyUpdateCheck(
  ...
  const merge =
    options.format === "claude" ? mergeClaudeHooks : mergeCursorHooks;
  let merged: string;
  try {
